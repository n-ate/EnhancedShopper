...
// @run-at       document-end
// @resource     //ajax.aspnetcdn.com/ajax/jQuery/jquery-3.2.1.js
// @supportURL   https://github.com/n-ate/EnhancedShopper/issues/new
// ==/UserScript==
    ...
    function LoadScripts(){
        if(parent === window) {//is top frame
            ...
            script.onload = function() {
                window.__JQ = jQuery;
                $.noConflict(true);
                OnloadScripts(window.__JQ);
            };
            script.src = '//ajax.aspnetcdn.com/ajax/jQuery/jquery-3.2.1.js';
            document.getElementsByTagName('head')[0].appendChild(script);
        }
        else if(IsCrossDomainFrame()) console.log(location.href + '\n cross domain, ignore iframe');
        else {
            if(parent.hasOwnProperty('__JQ')) {
                console.log(location.href + '\n domain match, jquery ready, run script');
                OnloadScripts(parent.__JQ);
            }
            else {
                console.log(location.href + '\n domain match, jquery not ready');
                setTimeout(function(){
                    LoadScripts.apply(this);
                }, 0);
            }
        }
    }
    function OnloadScripts($, debug) {
        var es = {
        ...
        if(debug) window.ES = es;
        else es.insertPricePerOunce();
    }
    if(window.ENHANCEDSHOPPER === 'debug') OnloadScripts($, true);
    else LoadScripts();
})();